...
def test_run_module_failed(clear_sign_mock, f_config_msg_signer_ok):
    ...
    with pytest.raises(AnsibleFailJson) as result:
        clear_sign.run_module()
    assert result.value.args[0]["failed"] == True


@patch.multiple(basic.AnsibleModule, fail_json=fail_json)
@patch("pubtools.sign.signers.msgsigner.msg_clear_sign")
def test_run_module_exception(msg_clear_sign_mock):
    msg_clear_sign_mock.side_effect = [ValueError("No configuration file found ...")]
    set_module_args({})
    with pytest.raises(AnsibleFailJson) as result:
        clear_sign.run_module()
    assert result.value.args[0]["failed"] == True
